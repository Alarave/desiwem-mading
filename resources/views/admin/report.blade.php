...
@section('title', 'Laporan & Rekapitulasi Eksekutif - Mading GUNDAR')
...
<div class="container-fluid px-0">
  ...
  <div class="report-control-bar no-print">
    <div>
      ...
      <p class="text-muted small mb-0 mt-1">Dicetak otomatis dari Arsip Resmi Mading GUNDAR per {{ date('d F Y') }}</p>
    </div>
    ...
    <div class="report-kop">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
        <div class="d-flex align-items-center gap-3">
          ...
          <div>
            <h2 class="report-kop-title">SEKOLAH TINGGI GUNDAR</h2>
            <div class="report-kop-subtitle">UNIT PENGELOLA MAJALAH DINDING DIGITAL (ASESOR)</div>
            <p class="report-kop-meta">
              Jl. Margonda Raya No. 100, Depok, Jawa Barat | Surel: mading@gundar.ac.id | Portal: mading.gundar.ac.id
            </p>
          </div>
        </div>
    ...
    <div class="mb-4">
      ...
      <div class="report-filter-bar no-print">
        <div class="report-cat-pills">
          <button type="button" class="cat-pill-btn active" onclick="filterByCategory('ALL', this)">
            Semua ({{ $articles->count() }})
          </button>
          @foreach($categories as $cat)
            <button type="button" class="cat-pill-btn" onclick="filterByCategory('{{ $cat->name }}', this)">
      ...
      <div class="row align-items-end">
        <div class="col-7">
          ...
          <p class="text-muted small mb-0" style="line-height: 1.5;">
            Laporan rekapitulasi data ini diterbitkan secara elektronik oleh Sistem Informasi Majalah Dinding Digital (ASESOR) Sekolah Tinggi GUNDAR dan diakui sebagai arsip dokumentasi resmi.
          </p>
        </div>
...
function copySummaryText() {
  const text = `LAPORAN REKAPITULASI MADING ASESOR
Sekolah Tinggi GUNDAR
Periode: {{ date('F Y') }}
...
Generated by Sistem Mading GUNDAR.`;
...
function downloadCSV() {
  ...
  @foreach($articles as $idx => $art)
    csvContent += `"${{ $idx + 1 }}","${{ str_replace('"', '""', $art->title) }}","${{ $art->category->name ?? 'Umum' }}","${{ $art->author->name ?? 'Admin' }}","${{ $art->created_at->format('d/m/Y H:i') }}"\n`;
  @endforeach
  ...
  link.setAttribute('download', `Rekapitulasi_Mading_GUNDAR_{{ date('Ymd') }}.csv`);
