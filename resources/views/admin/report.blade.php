...
@section('title', 'Laporan Publikasi Mading - DeSiWeM')
...
<div class="container-fluid px-0">
  ...
  <div class="report-control-bar no-print">
    <div class="report-control-left">
      <h1 class="report-control-title">Laporan Publikasi Mading</h1>
      <p class="report-control-meta">
        <i class="bi bi-calendar3"></i>
        <span>Periode {{ date('F Y') }} &bull; Diperbarui {{ date('d M Y, H:i') }} WIB</span>
      </p>
    </div>
    ...
    <div class="report-control-actions">
      ...
      <button type="button" class="btn-report-btn btn-report-primary" onclick="window.print()" title="Cetak atau Simpan PDF">
  ...
  <div class="report-sheet">
      ...
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
        <div class="d-flex align-items-center gap-3">
          ...
          <div>
            ...
            <div class="report-kop-subtitle">Portal Majalah Dinding Digital Sekolah</div>
        ...
        <div class="text-end d-none d-sm-block">
          <div class="p-2 border rounded bg-white d-inline-block text-center" style="border-color: #e4e4e7;">
            ...
            <span class="d-block font-monospace text-muted" style="font-size: 0.625rem; letter-spacing: 0.05em;">DESIWEM MADING</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="report-banner">
      <h3 class="report-banner-title">Rekapitulasi Artikel & Distribusi Konten Mading</h3>
      <p class="report-banner-period">
        Periode: {{ date('F Y') }} &bull; Dicetak: {{ date('d/m/Y H:i') }} WIB
      </p>
    </div>
...
function copySummaryText() {
  const text = `LAPORAN PUBLIKASI MADING
Portal DeSiWeM
Periode: {{ date('F Y') }}
...
- Kategori Terbanyak: {{ $topCategory->name ?? '-' }} ({{ $topCategory->articles_count ?? 0 }} artikel)
- Penulis Teraktif: {{ $topAuthor->name ?? 'Admin' }} ({{ $topAuthor->articles_count ?? 0 }} artikel)`;

  navigator.clipboard.writeText(text).then(() => {
    alert('Ringkasan data laporan berhasil disalin ke clipboard!');
  }).catch(() => {
